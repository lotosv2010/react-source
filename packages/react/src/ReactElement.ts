...
// 对照官方 jsx()：babel 自动运行时（automatic runtime）编译 JSX 时调用的入口，
// 与经典 createElement 的差异是 children 已经被合并进 config，不再用多余参数传递。
// jsx() 本身没有 DEV 校验（那些校验在 jsxDEV 里），因为编译器在 DEV 模式下会生成 jsxDEV 调用而非 jsx。
export function jsx(
  type: ElementType,
  config: any,
  maybeKey?: Key,
